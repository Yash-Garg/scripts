...
function debian_pkgs() {
    ...
    sudo apt-get install zsh npm mariadb-server firefox git 
}
...
function arch_pkgs() {
    ...
    sudo pacman -Syyu
    sudo pacman -S neofetch firefox filezilla telegram-desktop etcher git mariadb gnupg paper-icon-theme zsh npm
